...
def direction_arrow(value: float, neutral_epsilon: float = 1e-8) -> str:
    """用箭头表达指标方向。"""
...
def gradient_bar_colors(values: pd.Series):
    """
    为斜率柱状图生成红-黄-绿线性过渡色。
    负值偏红，0附近偏黄，正值偏绿。
    """
    clean = values.fillna(0.0)
    max_abs = max(float(clean.abs().max()), 0.01)
    norm = TwoSlopeNorm(vmin=-max_abs, vcenter=0.0, vmax=max_abs)
    return plt.cm.RdYlGn(norm(clean.to_numpy()))
...
def plot_aux_d_integrated_pdf(
    ...
    PDF 共两页：
    1. 综合D风险图：K线 + MA均线 + 斜率箭头 + 斜率柱状条 + RSP/VOO动量 + RSI + VIX；
    2. RSP/VOO 近252日滚动百分位。
    ...
    pdf_name = f"aux_d_risk_report_integrated_{file_date_str}_{asset.lower()}.pdf"
    pdf_path = OUTPUT_PATH / pdf_name

    metadata = {
        "Title": f"{asset} D Integrated Auxiliary Risk Report - {trade_date}",
        "Author": "VOO v3_cagr Quant System",
        "Subject": "Integrated candlestick, MA slope, RSP/VOO breadth and percentile",
        "Keywords": "VOO, MA150, RSP, breadth, candlestick, quantitative risk",
    }

    with PdfPages(pdf_path, metadata=metadata) as pdf:
        # -------------------- 第1页：综合D风险图 --------------------
        fig, (ax_price, ax_slope, ax_breadth, ax_rsi, ax_vix) = plt.subplots(
            5,
            1,
            figsize=(11.69, 8.27),
            gridspec_kw={"height_ratios": [3.0, 0.55, 0.9, 0.85, 0.85]},
            sharex=True,
        )
        ...
        y_min, y_max = ax_price.get_ylim()
        d_raw_mask = recent["aux_d_dual_weak_raw"].fillna(False).to_numpy()
        d_confirmed_mask = recent["aux_d_dual_weak_confirmed"].fillna(False).to_numpy()
        for i, is_raw in enumerate(d_raw_mask):
            if is_raw:
                color = "#e74c3c" if d_confirmed_mask[i] else "#f39c12"
                alpha = 0.14 if d_confirmed_mask[i] else 0.10
                ax_price.axvspan(i - 0.5, i + 0.5, color=color, alpha=alpha, linewidth=0)

        confirmed_x = x_values[d_confirmed_mask]
        if len(confirmed_x) > 0:
            ax_price.scatter(
                confirmed_x,
                np.full(len(confirmed_x), y_min + (y_max - y_min) * 0.05),
        ...
        latest_close = today[f"{asset}_close"]
        latest_ma = today[f"{asset}_MA"]
        latest_ma100 = today[f"{asset}_MA100"]
        latest_ma200 = today[f"{asset}_MA200"]
        latest_slope = today["aux_d_ma_log_daily_slope"]
        latest_breadth = today["aux_d_rsp_voo_rel_daily_mom"]
        slope_arrow = direction_arrow(latest_slope)
        breadth_arrow = direction_arrow(latest_breadth)
        price_note = (
            f"收盘 {latest_close:.2f}\n"
            f"距MA{p['ma_len']} {latest_close / latest_ma - 1:+.2%}\n"
            f"距MA100 {latest_close / latest_ma100 - 1:+.2%} | 距MA200 {latest_close / latest_ma200 - 1:+.2%}\n"
            f"MA斜率 {slope_arrow} {latest_slope * 100:+.4f}%/日\n"
            f"RSP/VOO动量 {breadth_arrow} {latest_breadth * 100:+.4f}%/日\n"
            f"风险等级：{risk_level}"
        )
        ax_price.text(
            0.985,
        ...
        ax_price.set_title(
            f"{asset} D辅助风险综合图 1/2：价格行为、MA斜率与市场广度\n"
        ...
        # MA150 斜率薄柱状条：红-黄-绿线性过渡
        slope_colors = gradient_bar_colors(slope_pct)
        ax_slope.bar(
            x_values,
            slope_pct.fillna(0.0),
            color=slope_colors,
            width=0.72,
            edgecolor="none",
            alpha=0.92,
        )
        ax_slope.axhline(0.0, color="#34495e", linestyle="--", linewidth=0.9)
        ax_slope.set_ylabel("MA斜率\n%/日", fontsize=8)
        ax_slope.grid(True, axis="y", linestyle="--", alpha=0.22)
        max_abs_slope = max(float(slope_pct.abs().max()), 0.01)
        ax_slope.set_ylim(-max_abs_slope * 1.25, max_abs_slope * 1.25)
        ...
        ax_breadth.axhline(0.0, color="#34495e", linestyle="--", linewidth=0.9, label="0轴")
        ax_breadth.fill_between(
            x_values,
            breadth_mom_pct.fillna(0.0),
            0.0,
            where=(breadth_mom_pct.fillna(0.0) >= 0),
            color="#27ae60",
            alpha=0.10,
        )
        ax_breadth.fill_between(
        ...
        ax_breadth.legend(loc="upper left", fontsize=8, ncol=2)
        ...
        ax_rsi.axhline(p["rsi_th"], color="#c0392b", linestyle="--", linewidth=1.0, label=f"RSI阈值 {p['rsi_th']}")
        ...
        ax_vix.axhline(p["vix_risk_th"], color="#c0392b", linestyle="--", linewidth=1.0, label=f"VIX风险阈值 {p['vix_risk_th']}")
        ax_vix.set_ylabel("VIX", fontsize=8)
        ...
        fig.text(
            0.02,
            0.022,
            "解读：主图看价格与均线，斜率柱看MA150坡度变化，RSP/VOO动量看市场参与面扩散；黄色/红色背景为D双弱状态。",
            fontsize=8.8,
        )
        fig.text(0.97, 0.022, "第 1 / 2 页", ha="right", fontsize=8.8)
        plt.tight_layout(rect=(0.02, 0.055, 0.98, 0.95))
        pdf.savefig(fig, bbox_inches="tight")
        plt.close(fig)
        ...
        ax.set_title(
            f"{asset} D辅助风险综合图 2/2：RSP/VOO近{percentile_window}日百分位\n"
            f"展示最近{chart_lookback}个交易日 | 截至 {trade_date}",
            fontsize=15,
            fontweight="bold",
        )
        ...
        fig.text(0.97, 0.025, "第 2 / 2 页", ha="right", fontsize=9)
        plt.tight_layout(rect=(0.02, 0.06, 0.98, 0.96))
        pdf.savefig(fig, bbox_inches="tight")
        plt.close(fig)
...
def generate_daily_report():
    ...
    for asset, p in ASSET_CONFIG.items():
        ...
        # todo 26-07-06
        # 生成VOO的D辅助风险综合PDF，不影响A模型交易逻辑
        try:
            aux_d_pdf_path = plot_aux_d_integrated_pdf(
                df=df,
                asset=asset,
                p=p,
