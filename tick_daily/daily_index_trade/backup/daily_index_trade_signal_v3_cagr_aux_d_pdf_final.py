...
def plot_aux_d_risk_pdf(
    ...
    pdf_name = f"aux_d_risk_report_{file_date_str}_{asset.lower()}.pdf"
    pdf_path = OUTPUT_PATH / pdf_name

    metadata = {
        "Title": f"{asset} D Auxiliary Risk Report - {trade_date}",
        "Author": "VOO v3_cagr Quant System",
        "Subject": "MA slope, RSP/VOO relative breadth and percentile",
        "Keywords": "VOO, MA150, RSP, breadth, quantitative risk",
    }
    ...
    with PdfPages(pdf_path, metadata=metadata) as pdf:
        ...
        ax.set_title(
            f"{asset} D辅助风险趋势图 1/2：趋势质量与相对广度\n"
        ...
        fig.text(0.97, 0.025, "第 1 / 2 页", ha="right", fontsize=9)
        ...
        ax.set_title(
            f"{asset} D辅助风险趋势图 2/2：RSP/VOO近{percentile_window}日百分位\n"
...
def generate_daily_report():
    ...
    for asset, p in ASSET_CONFIG.items():
        ...
        try:
            aux_d_pdf_path = plot_aux_d_risk_pdf(
                df=df,
                asset=asset,
                p=p,
