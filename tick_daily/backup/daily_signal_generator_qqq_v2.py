...
def generate_daily_report():
    ...
    # todo: 26-05-06 新增：当前箱体区间 + 三次加仓执行计划
    # ==================== 新增：箱体区间 + 执行建议 ====================
    recent = df.tail(60)
    support = recent['QQQ_close'].min()
    resistance = recent['QQQ_close'].max()
    # 保持与 VOO 版本代码结构完全一致，即使文本中暂不使用该变量
    atr = (recent['QQQ_high'] - recent['QQQ_low']).mean()
...
"""
    ...
    # ==================== Grok 风格报告 ====================
    # todo: 26-05-06 更新：加入 exec_suggestion 并优化了 MA 长度的动态显示
    grok_text = f"""📅 【{date_str} QQQ 中长期量化信号】
{'=' * 55}
趋势框架：{'✅ 多头' if base_trend else '❌ 空头'} (QQQ vs MA{p['ma_len']})
...
"""
    ...
    print(f"\n🎉 今日 QQQ 报告生成完成！所有文件均保存在：{OUTPUT_PATH}")
