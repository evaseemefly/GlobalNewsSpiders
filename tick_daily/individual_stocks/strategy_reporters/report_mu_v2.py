...
# ==================== 4. 实盘状态辅助函数 ====================
def normalize_live_state(live_state: dict | None) -> dict:
    """
    live_state 建议格式：
    {
        "stage": 0 or 1,
        "cost_price": 0.0,
        "shares": 0,
        "cash": None,                 # 可选：真实账户中准备给 MU 的可用资金
        "target_position_pct": 0.15,   # 可选：MU 在美股/科技账户中的目标上限
    }
    """
...
def run_backtest(df: pd.DataFrame, config: dict):
    ...
    cerebro.addstrategy(
        ...
        exit_period=config.get("exit_period", 40),
...
def generate_report_figure(
    ...
    exit_p = config.get("exit_period", 40)
...
def print_next_day_signals_v2(
    ...
    # 实盘分层仓位参数：默认首笔 35%，目标上限 95% 只是策略内资金池上限，不等于个人总账户上限
    ...
    print("   ⚠️ 以上只是历史模拟状态，不代表你现在的真实 MU 仓位。")
    ...
    if live_stage == 0 and bt_pos_ratio > 10:
        ...
        print("   若你真实账户已经买入 MU，请把 live_state 改为 stage=1 并填入成本和股数。")
    ...
    if live_stage is not None and (live_stage > 0 or live_shares > 0):
        print("   🔴 【趋势持仓防守监控】")
        print("      - 当前策略状态：实盘已有 MU，不能再生成空仓买入指令。")
    ...
    # ========== 实盘空仓：判断是否入场 ==========
        ...
        if current_price >= donchian_upper:
            ...
            print(f"      - 第一笔建议：动用 MU 计划资金池的 {first_tranche_pct * 100:.0f}% 作为试探仓。")
...
# ==================== 8. 主函数 ====================
def generate_strategy_report(ticker: str = "MU", config: dict | None = None):
...
if __name__ == "__main__":
    # 你可以按真实账户情况修改 live_state：
    # - 如果真实账户空仓：stage=0, cost_price=0.0, shares=0
    # - 如果真实账户已持仓：stage=1, cost_price=你的成本, shares=你的股数
    mu_v2_config = {
        "entry_period": 30,
        "exit_period": 40,
        "alloc_pct": 0.95,       # 回测策略资金池理论暴露
        ...
        "live_state": {
            ...
            # 可选：如果你想让系统估算第一笔买入股数，填入给 MU 预留的资金池
            # "cash": 15000,
    ...
    generate_strategy_report("MU", config=mu_v2_config)
