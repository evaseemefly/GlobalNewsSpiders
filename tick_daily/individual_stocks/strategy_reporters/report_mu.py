...
class MultiStageStrategy(bt.Strategy):
    ...
    def notify_order(self, order):
        ...
        if order.status in [order.Completed]:
            if order.isbuy():
                print(f"   ↳ ⚡ [历史回测成交] 🟢 买入突破 | 均价: ${order.executed.price:.2f} | 数量: {order.executed.size} 股")
                self.stage = 1
                self.entry_price = order.executed.price
            elif order.issell():
                print(f"   ↳ ⚡ [历史回测成交] 🔴 破位卖出 | 均价: ${order.executed.price:.2f} | 数量: {abs(order.executed.size)} 股")
                self.stage = 0
                self.entry_price = 0.0
            self.order = None
...
# ==================== 3. 完美复刻专业报告图 (8轴联动) ====================
def generate_strategy_report(ticker: str = "MU", config: dict = None):
    print(f"🚀 开始生成 {ticker} 动量趋势追踪最优策略专业报告图 (含净值对比)...")
    ...
    table_data = [
        ...
        ["资金头寸风控", f"最大全仓暴露比例: {p.alloc_pct * 100:.0f}%", "开仓资金分配: 一把推(无备用弹药)", "ATR波动率调配: 暂关闭"],
        ["移动退出矩阵", f"唐奇安下轨周期: {p.exit_period} 天", "平仓触发机制: 跌破下轨无条件清仓", "固定/硬目标止盈: 未启用"]
    ]
    ...
    print(f"\n✅ {ticker} 专属 8轴动量趋势研报图 (含净值对比曲线) 成功生成！")
...
if __name__ == '__main__':
    mu_best_config = {'entry_period': 30, 'exit_period': 40, 'alloc_pct': 0.95, 'verbose': True}
    generate_strategy_report("MU", config=mu_best_config)