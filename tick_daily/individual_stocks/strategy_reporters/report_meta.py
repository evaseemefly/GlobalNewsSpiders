...
# ==================== 2. META 专属：闪击网格 + 移动追踪止盈策略 ====================
class MultiStageStrategy(bt.Strategy):
    params = (
        ('rsi_entry_th', 50),
        ('rsi_exit_th', 80),
        ('drop1_pct', 0.05),  # ⚡️ 闪击网格：跌5%极速加仓
        ('drop2_pct', 0.05),  # ⚡️ 闪击网格：跌5%直接打满
        ('ma_period', 150),
        ('initial_alloc', 0.30),
        ('add1_alloc', 0.30),
        ('add2_alloc', 0.40),
        ('update_ref_on_add', False),  # 固定基准

        ('profit_target_pct', 0.15),  # 追踪激活线：15%
        ('trailing_drop_pct', 0.08),  # 回撤清仓线：8% (宽容回撤容忍度)
...
# ==================== 3. 完美复刻专业报告图 (8轴联动) ====================
def generate_strategy_report(ticker: str = "META", config: dict = None):
    print(f"🚀 开始生成 {ticker} 最优策略专业报告图 (闪击网格追踪止盈版)...")
    ...
    if config:
        cerebro.addstrategy(
            ...
            trailing_drop_pct=config.get('trailing_drop_pct', 0.08),
    ...
    table_data = [
        ["入场与风控", f"RSI 抄底阈值: < {params.rsi_entry_th}", f"长线均线过滤: 站上 MA{params.ma_period}",
         f"破位止损线: MA均线 -15%"],
        ["闪击网格", f"初始底仓: {params.initial_alloc * 100:.0f}%",
         f"极速一档 (跌{params.drop1_pct * 100:.0f}%): {params.add1_alloc * 100:.0f}%",
         f"极速满仓 (跌{params.drop2_pct * 100:.0f}%): {params.add2_alloc * 100:.0f}%"],
...
def print_next_day_signals(strat, ticker, current_date, current_price, live_state=None):
    ...
    if live_state is not None:
        ...
        last_buy_price = live_state['cost_price']
    ...
    if stage > 0:
        print(f"   当前持仓成本锚点: ${cost_price:.2f}")
...
if __name__ == '__main__':
    ...
    generate_strategy_report("META", config=meta_config)