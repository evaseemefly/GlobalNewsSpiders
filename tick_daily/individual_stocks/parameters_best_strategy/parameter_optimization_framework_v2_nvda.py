        ...
        if self.stage > 0:
            ...
            if self.params.profit_target_pct > 0 and self.position and self.position.size != 0:
                ...
                if price >= avg_price * (1 + self.params.profit_target_pct):
                    if self.params.verbose:
                        print(
                            f"[{date}] Stage {self.stage} → 清仓 (成本止盈 {self.params.profit_target_pct * 100:.0f}% @ {price:.2f})")
                    self.order = self.close()
                    self.stage = 0
                    return
...
def run_optimization(ticker: str, verbose_strategy: bool = False):
    ...
    # cerebro.optstrategy(
    #     MultiStageStrategy,
    #     rsi_entry_th=[45, 50, 55],  # 趋势股回调浅，提高进场门槛（容易上车）
    #     rsi_exit_th=[75, 80, 85],  # 动量强，允许指标严重超买后再卖出
    #     drop1_pct=[0.03, 0.05, 0.08],  # 加仓阈值缩小，牛市中很少有深调
    #     drop2_pct=[0.05, 0.08, 0.12],
    #     ma_period=[100, 150, 200],  # 引入更短的均线跟随趋势
    #     update_ref_on_add=[True, False],
    #     profit_target_pct=[0.0, 0.30, 0.50],  # 目标利润拉大，或者设为0让RSI主导
    ...
    cerebro.optstrategy(
        MultiStageStrategy,
        # === 针对 NVDA 超级趋势股的激进设置 ===
        rsi_entry_th=[55, 60, 65, 70],  # 大幅提高！允许 RSI 更高时就入场（NVDA 趋势中 RSI 常年 60-80）
        rsi_exit_th=[85, 90, 95],  # 极度宽松，让它超买更久再卖（甚至接近不靠 RSI 止盈）

        drop1_pct=[0.01, 0.02, 0.03, 0.05],  # 更浅！匹配 NVDA 实际回调深度
        drop2_pct=[0.03, 0.05, 0.08, 0.10],

        ma_period=[50, 100, 150, 200],  # 更短 MA，更快捕捉趋势启动
        update_ref_on_add=[True, False],  # 两种都测，链式更新大概率更好

        profit_target_pct=[0.0, 0.8, 1.2, 2.0],  # 大幅拉高！允许持仓翻 1-2 倍再考虑止盈
        verbose=[verbose_strategy]
    )
