...
# 修复 Mac/Linux 下的中文字体显示问题 (如果是纯英文图表可注释掉)
...
def clean_bad_price_data(df: pd.DataFrame) -> pd.DataFrame:
    """
    # todo 26-04-30 核心新增：移植防极端异常值清洗引擎。
    无情抹除一切 <=0, -999.0 以及不合理的极小值 (<1000)，
    防止波动率出现 5 亿倍的计算误差和持续跃迁。
    """
    df = df.copy()
    ...
    # 如果某一行4个价格全变成 NaN 了，直接抛弃
    df = df.dropna(subset=price_cols, how='all')

    # 局部断裂修复：使用 ffill 把前一分钟的价格拉平过来
...
def calculate_gk_volatility(df: pd.DataFrame, window_size: int = 60) -> pd.DataFrame:
    ...
    # todo 26-04-30 修复：确保平方根内不出现负数
    df_calc[vol_col_name] = np.sqrt(
        np.maximum(df_calc['GK_variance'].rolling(window=window_size, min_periods=1).mean(), 0))
    return df_calc
...
def plot_comprehensive_dashboard(df: pd.DataFrame, window_size: int, output_dir: Path, vol_type: str = 'YZ'):
    ...
    # todo 26-04-30 核心画图防卫：过滤掉极小值和 -999，彻底解决图表向下扎针的问题
...
def process_comprehensive_data(root_path: Path, window_size: int = 60):
    ...
    # todo 26-04-30 核心修复：引入硬性下限清洗机制，彻底解决 yfinance 带来的极端脏数据
    df_gold = clean_bad_price_data(df_gold)
    df_gold['timestamp_utc'] = df_gold['timestamp_utc'].dt.floor('5min')

    df_gold = df_gold.sort_values('timestamp_utc').drop_duplicates('timestamp_utc', keep='last')

    # 强制时间轴对齐 (填补可能断连的5分钟)
    df_gold = df_gold.set_index('timestamp_utc').resample('5min').asfreq().reset_index()
    ...
    if not macro_files:
        ...
        df_macro['timestamp_utc'] = pd.to_datetime(df_macro['timestamp_utc']).dt.floor('5min')
    ...
    # C. 合并与对齐 (以连续的黄金时间轴为基准)
    print(f"🔗 正在合并黄金与高频宏观数据...")
    df_all = pd.merge(df_gold, df_macro, on='timestamp_utc', how='left')

    # D. 核心延续逻辑：废除暴力填 NaN 和 -999，采用平滑延续策略
    # todo 26-04-30 修复：删除暴力的 is_weekend 和 is_dead = NaN 的判断。
    # 采用前向填充 (ffill) 延续宏观数据和收盘价，开/高/低等于收盘价。

    # 填补宏观指标断层
    df_all[['DXY', 'US10Y', 'VIX']] = df_all[['DXY', 'US10Y', 'VIX']].ffill()

    # 填补黄金价格断层 (形成方差为0的静止区域)
    ...
    # todo 26-04-30 终极修复：绝对禁止在保存前对价格执行 fillna(-999)！
    # 因为我们在 D 步已经平滑填充了所有数据，这里不需要再补 -999，直接保存即可。
    output_feature_dir.mkdir(parents=True, exist_ok=True)
    df_final.to_csv(csv_name, index=False, encoding='utf-8-sig')
    print(f"💾 合并特征文件已安全保存: {csv_name.name}")
