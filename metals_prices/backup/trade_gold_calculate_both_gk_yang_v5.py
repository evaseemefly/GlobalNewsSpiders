...
核心改进（v5 版本）：
  - 彻底抛弃强制填充 -999 和 `is_dead_feed = NaN` 的暴力做法。
  - 使用智能前向填充 (ffill)，让停盘/断连期间的收盘价自然延续，开高低等于收盘价。
  - 完美兼容 GK 和 YZ 公式：当价格静止时，波动率平滑衰减为 0，不再产生 -999 和空缺。
"""
...
def clean_bad_price_data(df: pd.DataFrame) -> pd.DataFrame:
    """
    针对 yfinance 偶尔返回 -999 / 0 / 负值 / 极值的严重坏数据进行清洗
    """
    df = df.copy()
    price_cols = ['open', 'high', 'low', 'close']

    print("🧹 开始执行坏数据清洗（-999 / 0 / 负值）...")
    for col in price_cols:
        # 标记异常值（小于等于0，或等于-999，或过于离谱的极大值）
        bad_mask = (df[col] <= 0) | (df[col] == -999) | (df[col] > 10000)
        bad_count = bad_mask.sum()
        if bad_count > 0:
            print(f"   发现 {bad_count} 个异常值 → {col}")
            # 使用正常数据进行前向填充，再后向填充兜底
            df[col] = df[col].where(~bad_mask).ffill().bfill()

    # 额外安全：如果某一行所有价格都是空，直接删除
    df = df.dropna(subset=price_cols, how='all')
    print(f"✅ 坏数据清洗完成，剩余记录数: {len(df)}")
    return df
