...
def recover_expected_bar_from_raw_include(
    ...
    """
    todo 26-09-15 修正下载bug
    当 auto_adjust=True 的最新交易日 OHLC 无效/缺失时，使用 auto_adjust=False 恢复“仅最新一根”日 K。

    返回字典字段：success / bar / reason / raw_hist。
    成功时 bar 只包含 expected_trade_date 一行；绝不以 raw window 覆盖既有 adjusted 历史。
    """
    expected = pd.Timestamp(expected_trade_date).normalize()
    ...
    if raw_hist.empty:
        return {
            "success": False,
            "bar": pd.DataFrame(),
            "reason": "raw daily 返回空数据",
            "raw_hist": raw_hist,
        }

    # todo 26-09-15 修正下载bug
    # 先判断 expected 是否真正存在，再检查 corporate action，避免把“缺少预期日”误标为 corporate action。
    if expected not in raw_hist.index:
        ...
        return {
            "success": False,
            "bar": pd.DataFrame(),
            "reason": f"raw daily 未返回预期交易日 {expected:%Y-%m-%d}",
            "raw_hist": raw_hist,
        }
    ...
    try:
        ...
        return {
            "success": False,
            "bar": pd.DataFrame(),
            "reason": f"raw daily 提取/清洗失败: {exc}",
            "raw_hist": raw_hist,
        }

    if issues:
        save_data_quality_debug_include(
            symbol, raw_hist, f"{debug_tag}_invalid", current_date_str
        )

    if clean_bar.empty or expected not in clean_bar.index:
        reason = "；".join(issues) if issues else "raw daily 预期日 bar 未通过 OHLCV 校验"
        return {
            "success": False,
            "bar": pd.DataFrame(),
            "reason": reason,
    ...
    # 只返回预期交易日这一根 raw bar；调用方负责与 adjusted 历史合并并再次做 clone/逻辑校验。
    return {
        "success": True,
        "bar": clean_bar.loc[[expected]].copy(),
        "reason": "raw daily 最新交易日恢复成功",
        "raw_hist": raw_hist,
    }
...
def probe_yahoo_daily_source_include(
    ...
    """
    todo 26-09-15 修正下载bug
    在正式下载全部股票前，对 Yahoo 最新日线做源级预检，并加入 adjusted -> raw daily 恢复链。

    - SOURCE_READY: 多数探针已经可靠达到 expected_trade_date；
    - SOURCE_PENDING: adjusted 与 raw fallback 都无法让多数探针达到 expected_trade_date，
      但多数探针一致停在更早的可信交易日；
    - SOURCE_ERROR: 探针结果分裂/不足，不能安全推断统一可信截止日。

    关键修正：yfinance 1.2.0 下 auto_adjust=True 可能出现“Volume 正常但 OHLC 全 NaN”；
    此时先用 auto_adjust=False 仅恢复 expected_trade_date，再决定是否 SOURCE_PENDING。
    """
    ...
    for symbol in probe_symbols:
        ...
        try:
            ...
            # todo 26-09-15 修正下载bug
            # adjusted 最新 bar 缺失/无效/克隆时，先尝试 raw daily 恢复 expected 日，
            # 不再直接把整个 Yahoo 源判成 SOURCE_PENDING。
            if needs_raw_recovery:
                ...
                recovery = recover_expected_bar_from_raw_include(
                    ...
                    debug_tag="source_probe_raw_recovery",
                )
                if recovery.get("success"):
                    ...
                    if final_clone:
                        ...
                        result["recovery_mode"] = "RAW_DAILY"
                        result["raw_daily_recovered"] = True
                else:
                    result["issues"].append(
                        f"raw fallback失败: {recovery.get('reason', '未知原因')}"
                    )

            if clean.empty:
                raise RuntimeError("adjusted/raw 清洗后均无可信 bar")

            # 若未使用 raw fallback，最终仍检查 adjusted 最新 bar 是否克隆。
            ...
            if latest_date == expected_trade_date:
                result["status"] = "READY_RAW" if result["raw_daily_recovered"] else "READY"
            ...
            if result["issues"]:
                save_data_quality_debug_include(
                    symbol, hist, "source_probe_adjusted_raw", current_date_str
                )

            print(
                f"   • {symbol}: {result['status']} | "
                f"可信截止 {result['latest_trusted_date']} | 模式 {result['recovery_mode']}"
            )
        except Exception as exc:
    ...
    if len(valid_latest_dates) >= quorum:
        ...
        if common_count >= quorum:
            ...
            if matching_modes:
                if all(mode == "RAW_DAILY" for mode in matching_modes):
                    source_mode = "RAW_FALLBACK"
                elif any(mode == "RAW_DAILY" for mode in matching_modes):
                    source_mode = "MIXED"
                elif all(mode == "ADJUSTED" for mode in matching_modes):
...
def save_data_quality_status_include(status_map: dict, current_date_str: str) -> None:
    """
    todo 26-09-15 修正下载bug
    保存本轮个股数据质量状态，并记录 Yahoo source_mode（ADJUSTED / RAW_FALLBACK / MIXED）。
    """
...
def fetch_daily_stock_data_include(tickers: list, start_date: str = "2020-01-01"):
    """
    todo 26-09-15 修正下载bug
    下载并更新个股日线。主历史保持 adjusted 口径；当最新 adjusted bar 异常时，
    优先用 raw daily（auto_adjust=False）仅恢复 expected_trade_date 一根，再进入原有重拉/降级逻辑。

    数据质量链：
    1) 个股 OHLC 禁止 ffill；
    2) adjusted 全历史 OHLCV 完整性/逻辑校验；
    3) 最新 bar 克隆异常检测；
    4) SPY/QQQ/MSFT 源级预检也执行 adjusted -> raw fallback；
    5) adjusted 最新日异常时，raw daily 只恢复 expected 日，且 corporate action 日禁止直接拼接；
    6) raw 恢复失败后，才进入新 Session + adjusted 短窗口重拉；
    ...
    for symbol in tickers:
        ...
        for attempt in range(max_retries):
            try:
                ...
                # todo 26-09-15 修正下载bug
                # 只要 adjusted 最新日未达到 expected，或最新 bar 命中克隆异常，先尝试 raw daily 仅恢复 expected 一根。
                needs_raw_recovery = (
                    clean_df.empty
                ...
                if needs_raw_recovery:
                    ...
                    print(
                        f"      🔄 {symbol} adjusted 最新日异常，尝试 raw daily 恢复: "
                        + "；".join(reason_parts)
                    )
                    ...
                    if recovery.get("success"):
                        ...
                        if recovery_clone_reason:
                            print(f"      ❌ {symbol} raw恢复后仍命中克隆异常: {recovery_clone_reason}")
                            ...
                            raw_daily_recovered_any = True
                            adjusted_clone_reason = None
                            print(
                                f"      ✅ {symbol} raw daily 恢复成功: {expected_trade_date_str} "
                                f"(仅替换最新一根，历史仍保持 adjusted 口径)"
                            )
                        else:
                            last_reason = (
                                f"raw daily 恢复后仍未达到市场预期 {expected_trade_date_str}"
                            )
                            print(f"      ⚠️ {symbol} {last_reason}")
                    else:
                        last_reason = f"raw daily 恢复失败: {recovery.get('reason', '未知原因')}"
                        print(f"      ⚠️ {symbol} {last_reason}")
                ...
                # todo 26-09-15 修正下载bug
                # raw fallback 失败后，才进入原有 adjusted 短窗口重拉；SOURCE_PENDING 下若已达到 source_available，
                # 则不进行无意义的 4 次完整重试。
                ...
                if actual_date == expected_trade_date:
                    if raw_daily_recovered_any:
                        final_status = "RAW_DAILY_RECOVERED"
                        final_message = (
                            "adjusted 最新日异常，已使用 auto_adjust=False 仅恢复最新一根；"
                            "历史数据仍保持 adjusted 口径，最终达到市场预期交易日"
                        )
                    elif retry_used_any:
                    ...
                    final_message = (
                        f"adjusted/raw 均未恢复到市场预期 {expected_trade_date_str}；"
    ...
    # todo 26-09-15 修正下载bug
    market_latest_statuses = {"OK", "RETRY_RECOVERED", "RAW_DAILY_RECOVERED"}
    ...
    print(
        ...
        f"(其中 raw 恢复 {raw_recovered_count}) | "
...
def download_stocks_main(target_stocks=None, start_date="2023-01-01"):
    """
    todo 26-09-15 修正下载bug
    个股下载器入口。返回每个标的的数据质量状态，包含 adjusted/raw 恢复模式及 market/source 两层日期。
    """
...
PIPELINE_VERSION = "pipeline_260915_v2"
...
def _draw_four_dim_cover(snapshot: Dict[str, Any], data_quality: Optional[Dict[str, Any]] = None):
    ...
    if quality_total > 0:
        ...
        market_latest = sum(
            ...
            if item.get("data_status") in {"OK", "RETRY_RECOVERED", "RAW_DAILY_RECOVERED"}
        )
        # todo 26-09-15 修正下载bug
        raw_recovered = sum(
            1 for item in quality_map.values()
        ...
        quality_text = (
            f"个股数据: 市场最新 {market_latest}/{quality_total}（raw恢复 {raw_recovered}）  |  "
    ...
    with legacy_reporter.PdfPages(output_path) as pdf:
        ...
        for ticker in target_stocks:
            ...
            if dq_status == "OK":
                ...
                data_line = f"最新可信交易日 {dq_actual} | 数据状态：adjusted重拉后恢复 | 市场预期 {dq_expected}"
                data_color = "#b54708"
                data_weight = "normal"
            # todo 26-09-15 修正下载bug
            elif dq_status == "RAW_DAILY_RECOVERED":
                data_line = (
                    f"最新可信交易日 {dq_actual} | 数据状态：Raw日线恢复 | "
...
def run_pipeline(is_need_download: bool = True, refresh_risk: bool = True) -> Optional[Path]:
    """
    todo 26-09-15 修正下载bug
    四维整合主流水线：加入 adjusted->raw 最新日恢复、Yahoo 源级预检及逐页可信日期显示。
    """
    print("=" * 68)
