...
def download_yf_history_include(
    ...
    """
    todo 26-09-09
    从 Yahoo/yfinance 下载日线候选数据。

    重要：Yahoo 返回值只视为“候选数据”，后续必须通过 OHLCV 完整性、最新性和异常 bar 校验后才能落盘。
    """
    ...
    kwargs = dict(
        ...
        interval="1d",
        auto_adjust=True,  # 显式固定当前价格口径，避免 yfinance 默认值变化造成历史口径漂移。
        actions=False,
        prepost=False,
        keepna=True,
    )
    ...
    try:
        ...
        # 兼容较旧 yfinance：若 keepna 等参数不被支持，则退回核心参数。
...
# todo 26-09-14
def download_yf_intraday_session_bar_include(symbol: str, session_date, fresh_session: bool = True) -> pd.DataFrame:
    """
    todo 26-09-14
    当 Yahoo 日线对已收盘交易日只返回 Volume、OHLC 为 NaN 时，
    用当日 1 小时 K（正规时段）合成一根候选日 K。
    这不是第二行情源，只是同一 Yahoo 源的更细周期兜底。
    """
    session_ts = pd.Timestamp(session_date).normalize()
    start_date = session_ts.strftime("%Y-%m-%d")
    end_date = (session_ts + pd.Timedelta(days=1)).strftime("%Y-%m-%d")
    session = get_downloader_custom_session() if fresh_session else None
    ticker_obj = yf.Ticker(symbol, session=session)
    try:
        hist = ticker_obj.history(
    ...
    if getattr(idx, "tz", None) is not None:
        ...
        # 正规时段约 9:30-16:00 ET；16:00 整点收盘 bar 通常不含。
        hist = hist.loc[(hour >= 9.5) & (hour < 16.0)].copy()
        hist.index = idx.tz_localize(None)
    if hist.empty or not set(["Open", "High", "Low", "Close"]).issubset(hist.columns):
        return pd.DataFrame()

    open_px = pd.to_numeric(hist["Open"], errors="coerce").dropna()
    high_px = pd.to_numeric(hist["High"], errors="coerce").dropna()
    low_px = pd.to_numeric(hist["Low"], errors="coerce").dropna()
    close_px = pd.to_numeric(hist["Close"], errors="coerce").dropna()
    vol = pd.to_numeric(hist["Volume"], errors="coerce").fillna(0)
    if open_px.empty or high_px.empty or low_px.empty or close_px.empty:
        return pd.DataFrame()

    bar = pd.DataFrame({
        "Open": [float(open_px.iloc[0])],
        "High": [float(high_px.max())],
        "Low": [float(low_px.min())],
        "Close": [float(close_px.iloc[-1])],
        "Volume": [float(vol.sum())],
    }, index=pd.DatetimeIndex([session_ts], name="Date"))
    return bar


# todo 26-09-14
def repair_unpublished_daily_bar_include(
        symbol: str,
        raw_df: pd.DataFrame,
        session_date,
) -> tuple:
    """
    todo 26-09-14
    若目标交易日日线缺失，或仅有成交量、OHLC 为空，尝试用 1 小时 K 合成后写回。
    返回 (raw_df, repaired: bool, note: str)。
    """
    if raw_df is None:
        return raw_df, False, ""
    session_ts = pd.Timestamp(session_date).normalize()
    ...
    c = f"{symbol}_close"
    v = f"{symbol}_volume"
    need_repair = session_ts not in raw_df.index
    if not need_repair:
        row = raw_df.loc[session_ts]
        price_missing = pd.isna(row[o]) or pd.isna(row[h]) or pd.isna(row[l]) or pd.isna(row[c])
        vol_ok = pd.notna(row[v]) and float(row[v]) > 0
        need_repair = bool(price_missing and vol_ok) or bool(price_missing)
    if not need_repair:
        return raw_df, False, ""

    hourly = download_yf_intraday_session_bar_include(symbol, session_ts, fresh_session=True)
    if hourly.empty:
        return raw_df, False, "小时线兜底未拿到可用 bar"
    patched = extract_stock_ohlcv_include(hourly, symbol)
    if patched.empty:
        return raw_df, False, "小时线合成后仍无 OHLCV"
    out = raw_df.copy() if not raw_df.empty else patched.copy()
    out.loc[session_ts, patched.columns] = patched.iloc[0]
    note = (
        f"{session_ts:%Y-%m-%d} 日线 OHLC 未定稿，"
        f"已用 1h 合成 O={patched.iloc[0][o]}, H={patched.iloc[0][h]}, "
        f"L={patched.iloc[0][l]}, C={patched.iloc[0][c]}, V={patched.iloc[0][v]}"
    )
    return out.sort_index(), True, note
...
def probe_yahoo_daily_source_include(
    ...
    """
    todo 26-09-10
    在正式下载全部股票前，对 Yahoo 最新日线做“源级预检”。

    目的：把“交易所理论上已经收盘”与“Yahoo 日线源当前已经发布到哪一天”分开。
    - SOURCE_READY: 多数探针标的已经有 expected_trade_date 的合格日 K；
    - SOURCE_PENDING: 多数探针标的一致停在更早的同一可信交易日；
    - SOURCE_ERROR: 探针结果分裂/不足，不能安全推断 Yahoo 的统一可信截止日。

    探针仅用于确定 Yahoo 当前统一可用日期，不会降低 OHLCV/克隆 bar 的个股质量标准。
    """
    ...
    for symbol in probe_symbols:
        ...
        try:
            ...
            raw, repaired, repair_note = repair_unpublished_daily_bar_include(
                symbol, raw, expected_trade_date
            )
            if repair_note:
                print(f"      🔧 预检 {symbol}: {repair_note}")
            ...
            if clone_reason:
                issues = list(issues) + [clone_reason]
                result["issues"] = issues
                # 探针中克隆最新 bar 不能视为可信；隔离后再确定源可用日期。
                clean = clean.iloc[:-1].copy()

            if clean.empty:
                raise RuntimeError("清洗后无可信 bar")

            latest_date = clean.index[-1].normalize()
            result["latest_trusted_date"] = latest_date.strftime("%Y-%m-%d")
            result["status"] = "READY" if latest_date == expected_trade_date else "BEHIND"
            valid_latest_dates.append(result["latest_trusted_date"])

            if issues:
                save_data_quality_debug_include(
                    symbol, hist, "source_probe_raw", current_date_str
                )

            print(
                f"   • {symbol}: {result['status']} | "
                f"可信截止 {result['latest_trusted_date']}"
            )
        except Exception as exc:
    ...
    if len(valid_latest_dates) >= quorum:
        # 不额外依赖 collections.Counter，保持自包含。
        ...
        if common_count >= quorum:
            ...
            source_status = (
                "SOURCE_READY"
                if source_ts >= expected_trade_date
                else "SOURCE_PENDING"
            )
    ...
    print(
        ...
        f"Yahoo可信截止 {source_available_date or '无法统一判定'}"
    )
...
def fetch_daily_stock_data_include(tickers: list, start_date: str = "2020-01-01"):
    """
    todo 26-09-10
    下载并更新个股日线，执行数据质量防线并加入 Yahoo 源级预检：

    1) 个股 OHLC 禁止 ffill；
    2) OHLCV 完整性/逻辑校验；
    3) 最新 bar 克隆异常检测；
    4) 先用 SPY/QQQ/MSFT 判断 Yahoo 当前统一可信截止日；
    5) 个股异常后用新 Session + 短窗口重拉；
    6) 区分 market_expected_date 与 source_available_date；
    7) 只有可信数据才原子覆盖 master，真正 stale 时保留 last-known-good。

    关键原则：SOURCE_PENDING 不是个股错误。若 Yahoo 整体尚未发布到市场理论最新日，
    个股只要达到 Yahoo 当前可信截止日即可进入报告，但必须明确标注“数据源待更新”。
    """
    ...
    # Yahoo 源整体待更新时，以其多数探针共同确认的日期作为“当前源可信下限”；
    # 若源级预检不一致/失败，则保持最严格模式，仍要求达到市场理论日期。
    ...
    for symbol in tickers:
        ...
        status = {
            "expected_trade_date": expected_trade_date_str,  # 兼容旧字段
        ...
        for attempt in range(max_retries):
            try:
                ...
                if hist.empty:
                    raise RuntimeError("Yahoo 返回空历史数据")

                raw_df = extract_stock_ohlcv_include(hist, symbol)
                raw_df, repaired, repair_note = repair_unpublished_daily_bar_include(
                    symbol, raw_df, expected_trade_date
                )
                if repair_note:
                    print(f"      🔧 {symbol}: {repair_note}")
                # 不允许任何未来于“市场理论最新完整交易日”的 bar 进入后续计算。
                raw_df = raw_df.loc[raw_df.index <= expected_trade_date]
                clean_df, issues = clean_stock_ohlcv_include(raw_df, symbol)

                for issue in issues:
                    print(f"      ⚠️ {symbol}: {issue}")

                if clean_df.empty:
                    raise RuntimeError("OHLCV 清洗后无有效数据")
                ...
                # todo 26-09-10
                # 只有“克隆 bar”或“连 Yahoo 当前可信截止日都没达到”才进行个股重拉。
                # SOURCE_PENDING 下 expected 日的无效占位 bar 被清掉后，只要 actual==source_available 就不重复重试。
                if clone_reason or stale_vs_source:
                    ...
                    if stale_vs_source:
                        reasons.append(
                            f"最新可信日期 {actual_date:%Y-%m-%d} 早于本轮最低可信截止 "
                            f"{required_trade_date:%Y-%m-%d}"
                        )
                    last_reason = "；".join(reasons)
                    print(f"      ⚠️ {symbol} 候选数据异常: {last_reason}")

                    save_data_quality_debug_include(
                        symbol, hist, f"attempt{attempt + 1}_raw", current_date_str
                    )
                    ...
                    save_data_quality_debug_include(
                        symbol, retry_hist, f"attempt{attempt + 1}_short_retry_raw", current_date_str
                    )

                    if not retry_hist.empty:
                        retry_raw = extract_stock_ohlcv_include(retry_hist, symbol)
                        retry_raw, _retry_repaired, retry_repair_note = repair_unpublished_daily_bar_include(
                            symbol, retry_raw, expected_trade_date
                        )
                        if retry_repair_note:
                            print(f"      🔧 {symbol} 重拉: {retry_repair_note}")
                        retry_raw = retry_raw.loc[retry_raw.index <= expected_trade_date]
                        retry_clean, retry_issues = clean_stock_ohlcv_include(retry_raw, symbol)
                        for issue in retry_issues:
                            print(f"      ⚠️ {symbol} 重拉: {issue}")
                ...
                if actual_date == expected_trade_date:
                    ...
                    final_message = (
                        "数据质量校验通过"
                        if not retry_used_any
                        else "异常后重拉恢复，最终达到市场预期交易日"
                    )
                elif (
                    ...
                    final_message = (
                        f"Yahoo日线源整体尚未发布到市场预期 {expected_trade_date_str}；"
                        f"本标的已达到 Yahoo 当前可信截止 {source_available_date:%Y-%m-%d}"
                    )
                else:
                    # 理论上前面的 freshness 检查已经拦截；保留防御分支。
                    final_status = "RETRY_RECOVERED" if retry_used_any else "OK"
    ...
    market_latest_count = sum(
        ...
        if item.get("data_status") in {"OK", "RETRY_RECOVERED"}
    )
...
def download_stocks_main(target_stocks=None, start_date="2023-01-01"):
    """
    todo 26-09-10
    个股下载器入口。返回每个标的的数据质量状态，并携带 market/source 两层日期供 PDF 显示。
    """
...
PIPELINE_VERSION = "pipeline_260910_v1"
...
def _draw_four_dim_cover(snapshot: Dict[str, Any], data_quality: Optional[Dict[str, Any]] = None):
    """
    todo 26-09-10
    绘制四维风险执行首页，并同时显示市场预期日、Yahoo 当前可信截止日及个股数据状态。
    """
    ...
    if quality_total > 0:
        ...
        fig.text(
            ...
            f"Yahoo可信截止: {source_available or 'N/A'}",
            fontsize=10.5,
            color=source_color,
            fontweight="bold",
        )
        ...
        quality_text = (
            f"个股数据: 市场最新 {market_latest}/{quality_total}  |  "
...
def generate_stock_report_with_four_dim(
    ...
    """
    todo 26-09-10
    复用旧绘图引擎生成四维整合版 PDF；每个个股页无论正常/待源更新/异常都显示最后可信交易日。
    """
    ...
    with legacy_reporter.PdfPages(output_path) as pdf:
        ...
        for ticker in target_stocks:
            ...
            if dq_status == "OK":
                data_line = f"最新可信交易日 {dq_actual} | 数据状态：OK（已达到市场预期 {dq_expected}）"
                data_color = "#067647"
                data_weight = "normal"
            elif dq_status == "RETRY_RECOVERED":
                data_line = f"最新可信交易日 {dq_actual} | 数据状态：重拉后恢复 | 市场预期 {dq_expected}"
                data_color = "#b54708"
                data_weight = "normal"
            elif dq_status == "SOURCE_PENDING":
                data_line = (
                    f"最新可信交易日 {dq_actual} | Yahoo日线待更新 | "
                    f"市场预期 {dq_expected} | Yahoo可信截止 {dq_source}"
                )
                data_color = "#b54708"
                data_weight = "bold"
            elif dq_status == "UNKNOWN":
                data_line = f"最新可信交易日 {dq_actual} | 数据状态：UNKNOWN（未加载本轮质量状态）"
                data_color = "#667085"
                data_weight = "normal"
            else:
                data_line = (
                    f"⚠ 最新可信交易日 {dq_actual} | 数据状态 {dq_status} | "
...
def run_pipeline(is_need_download: bool = True, refresh_risk: bool = True) -> Optional[Path]:
    """
    todo 26-09-10
    四维整合主流水线：加入 Yahoo 源级预检、SOURCE_PENDING 状态与逐页最后可信交易日显示。
    """
    print("=" * 68)
