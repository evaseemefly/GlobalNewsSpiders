...
def _four_dim_status_color(status: str) -> str:
    """四维灯号底板色：绿正常 / 黄预警 / 红风险。"""
    return {
        "green": "#1f9d55",
        "yellow": "#f0b429",
        "red": "#d64545",
    }.get(str(status).strip().lower(), "#98a2b3")


def _four_dim_status_fg(status: str) -> str:
    """黄底用深色字，其余用白字，保证对比度。"""
    return "#1d2939" if str(status).strip().lower() == "yellow" else "#ffffff"


def _wrap_cover_action(text: str, width: int = 14) -> str:
    """首页动作栏按字数换行；中文无空格时 textwrap 不会断开。"""
    raw = "".join(str(text or "N/A").split())
    if len(raw) <= width:
        return raw
...
def _draw_status_light_buttons(ax, x0: float, y0: float, votes: Dict[str, Any]) -> None:
    """在指定起点绘制价/利/信/波四个圆角色块按钮。"""
    ...
    specs = [
        ...
        ("macro", "利"),
        ("credit", "信"),
        ("volatility_breadth", "波"),
    ]
    btn_w, btn_h, gap = 0.038, 0.145, 0.007
    for idx, (key, label) in enumerate(specs):
        status = str((votes.get(key) or {}).get("status", "unknown")).lower()
        x = x0 + idx * (btn_w + gap)
        ax.add_patch(
            FancyBboxPatch(
                (x, y0),
                ...
                linewidth=0.6,
                facecolor=_four_dim_status_color(status),
                edgecolor="#ffffff",
        ...
        ax.text(
            ...
            fontsize=8,
            fontweight="bold",
            color=_four_dim_status_fg(status),
            zorder=5,
        )


def _draw_four_dim_cover(snapshot: Dict[str, Any]):
    """个股报告首页：四维状态改为绿/黄/红矩形灯号，不再使用 G/Y/R 简写。"""
    ...
    fig = plt.figure(figsize=(11.69, 8.27))
    fig.patch.set_facecolor("#f7f9fb")
    fig.text(0.06, 0.915, "个股多周期报告 · 四维风险执行首页", fontsize=24, fontweight="bold", color="#233044")
    fig.text(
        0.06,
        0.875,
    ...
    ax = fig.add_axes([0.05, 0.40, 0.90, 0.43])
    ax.set_xlim(0, 1)
    ax.set_ylim(0, 1)
    ax.axis("off")

    headers = ["代理", "防抖后等级", "四维状态", "US10Y动量(环比)", "HYG偏离MA60", "最终动作"]
    col_x = [0.012, 0.078, 0.268, 0.458, 0.638, 0.768]
    header_y, header_h = 0.78, 0.20
    row_h = 0.30
    row_ys = [0.45, 0.12]

    ax.add_patch(Rectangle((0.0, header_y), 1.0, header_h, facecolor="#344054", edgecolor="none", zorder=1))
    for x, title in zip(col_x, headers):
        ax.text(x, header_y + header_h / 2, title, color="white", fontsize=9.2, fontweight="bold", va="center", ha="left", zorder=2)

    for row_idx, asset in enumerate(["VOO", "QQQ"]):
        ...
        y = row_ys[row_idx]
        bg = "#ffffff" if row_idx == 0 else "#f2f4f7"
        ax.add_patch(
            FancyBboxPatch(
                (0.0, y),
                1.0,
        ...
        ax.text(col_x[0], y + row_h / 2, asset, fontsize=11, fontweight="bold", color="#233044", va="center", ha="left", zorder=3)
        ax.text(col_x[1], y + row_h / 2, execution.get("state", "N/A"), fontsize=9.2, color="#233044", va="center", ha="left", zorder=3)
        _draw_status_light_buttons(ax, col_x[2], y + (row_h - 0.145) / 2, votes)
        ax.text(
            col_x[3],
            y + row_h / 2,
        ...
        ax.text(
            col_x[5],
            y + row_h / 2,
            _wrap_cover_action(execution.get("action", "N/A")),
            fontsize=8.2,
            color="#344054",
            va="center",
            ha="left",
    ...
    # 灯号图例，放在表格正下方、规则区上方，避免和标题抢位。
    fig.text(0.06, 0.355, "灯号", fontsize=10, fontweight="bold", color="#233044")
    legend_ax = fig.add_axes([0.11, 0.342, 0.46, 0.028])
    legend_ax.set_xlim(0, 1)
    legend_ax.set_ylim(0, 1)
    legend_ax.axis("off")
    legend_items = [("green", "绿 · 正常"), ("yellow", "黄 · 预警"), ("red", "红 · 风险")]
    for i, (status, caption) in enumerate(legend_items):
        x = 0.02 + i * 0.33
        legend_ax.add_patch(Rectangle((x, 0.18), 0.07, 0.64, facecolor=_four_dim_status_color(status), edgecolor="none"))
        legend_ax.text(x + 0.09, 0.5, caption, fontsize=9, color="#475467", va="center", ha="left")
    fig.text(0.58, 0.355, "按钮顺序：价 / 利 / 信 / 波", fontsize=9, color="#667085")

    fig.text(0.06, 0.295, "执行规则", fontsize=15, fontweight="bold", color="#233044")
    fig.text(
        0.06,
        0.155,
        "Level 0：正常交易\n"
