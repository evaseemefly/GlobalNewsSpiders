...
def clean_stock_ohlcv_include(df_single: pd.DataFrame, symbol: str):
    ...
    invalid_nonpositive_price = (df[price_cols] <= 0).any(axis=1)
    invalid_volume = df[v].isna() | (df[v] <= 0)
    invalid_ohlc_logic = (
        (df[h] < df[o])
        | (df[h] < df[c])
        | (df[l] > df[o])
    ...
    if invalid_mask.any():
        ...
        issues.append(f"删除无效 OHLCV bar: {invalid_dates}")
...
# todo 26-09-09
def save_data_quality_debug_include(symbol: str, df_or_hist: pd.DataFrame, tag: str, current_date_str: str) -> Optional[Path]:
    """
    todo 26-09-09
...
def fetch_daily_stock_data_include(tickers: list, start_date: str = "2020-01-01"):
    """
    todo 26-09-09
    下载并更新个股日线，执行六层数据质量防线：

    1) 个股 OHLC 禁止 ffill；
    2) OHLCV 完整性/逻辑校验；
    3) 最新 bar 克隆异常检测；
    4) 异常后用新 Session + 短窗口重拉；
    5) 与预期最新美股交易日做新鲜度核验；
    6) 只有可信数据才原子覆盖 master，异常时保留 last-known-good。
    """
    ...
    print(f"🗓️ 预期最新完整美股交易日: {expected_trade_date_str}")
    ...
    for symbol in tickers:
        ...
        for attempt in range(max_retries):
            try:
                ...
                stale = actual_date < expected_trade_date

                if clone_reason or stale:
                    retry_used_any = True
                    ...
                    if stale:
                        reasons.append(
                            f"最新可信日期 {actual_date:%Y-%m-%d} 早于预期 {expected_trade_date_str}"
                        )
                    last_reason = "；".join(reasons)
                    print(f"      ⚠️ {symbol} 首次候选数据异常: {last_reason}")

                    save_data_quality_debug_include(
                        symbol,
                        hist,
                        f"attempt{attempt + 1}_raw",
                        current_date_str,
                    )

                    # 短窗口重新拉取，使用新的 Session，避免继续沿用第一次长区间请求的候选数据。
                    ...
                    save_data_quality_debug_include(
                        symbol,
                        retry_hist,
                        f"attempt{attempt + 1}_short_retry_raw",
                        current_date_str,
                    )
                ...
                if final_clone_reason:
                    ...
                    # 重拉仍异常：隔离最后一根可疑 bar，再判断是否因此变成 stale。
                ...
                if actual_date != expected_trade_date:
                    last_reason = (
                        f"数据新鲜度不通过：预期 {expected_trade_date_str}，"
                ...
                # 只有“最新可信日期 == 预期交易日”才计算指标并覆盖 master。
                ...
                status.update({
                    ...
                    "data_status": "RETRY_RECOVERED" if retry_used_any else "OK",
                    "retry_used": retry_used_any,
                    "latest_bar_suspect": latest_bar_suspect_any,
                    "master_updated": True,
                    "message": "数据质量校验通过" if not retry_used_any else "异常后重拉恢复，最终校验通过",
                })

                print(
                    f"   ✅ {symbol} 下载及特征计算成功: {len(df_single)} 条记录 | "
                    f"最新交易日 {actual_date:%Y-%m-%d} | 状态 {status['data_status']}"
                )
        ...
        if not success:
            # todo 26-09-09
            # 失败或 stale 时绝不覆盖已有 last-known-good master。
            ...
            if master_save_path.exists():
                ...
                message = (
                    f"保留既有 last-known-good master；"
                    f"预期 {expected_trade_date_str}，"
                    f"现有可信 {actual_trusted_date or 'N/A'}；"
                    f"本轮原因: {last_reason or '未知'}"
                )
            elif not final_candidate.empty:
                # 首次运行且没有旧 master：允许保存“已清洗但过期”的候选历史，必须标记 STALE_INITIALIZED。
                ...
                message = (
                    f"首次运行无旧 master，保存已清洗但过期的候选数据；"
                    f"预期 {expected_trade_date_str}，实际 {actual_trusted_date or 'N/A'}"
                )
            else:
            ...
            print(f"      预期最新交易日 : {expected_trade_date_str}")
            print(f"      实际可信交易日 : {actual_trusted_date or 'N/A'}")
            print(f"      数据状态       : {status_code}")
            print(f"      说明           : {message}")
            print("      本次技术指标/PDF不得视为预期交易日的最新信号。")
            print("   " + "=" * 60)
    ...
    print(
        f"\n✅ 个股下载阶段完成：{ok_count}/{len(status_map)} 个标的数据为最新可信状态。"
...
def download_stocks_main(target_stocks=None, start_date="2023-01-01"):
    """
    todo 26-09-09
    个股下载器入口。返回每个标的的数据质量状态，供四维 PDF 首页和个股页面显示数据新鲜度。
    """
...
def _draw_four_dim_cover(snapshot: Dict[str, Any], data_quality: Optional[Dict[str, Any]] = None):
    """
    todo 26-09-09
    绘制四维风险执行首页，并同步显示个股下载数据的新鲜度/质量汇总。
    """
    ...
    quality_bad = quality_total - quality_ok
    if quality_total > 0:
        quality_text = f"个股数据质量: {quality_ok}/{quality_total} 最新可信"
        if quality_bad > 0:
            quality_text += f"  |  {quality_bad} 个标的过期/异常（详见对应个股页）"
        fig.text(
            0.06,
            0.815,
            quality_text,
            fontsize=10.5,
            color="#b42318" if quality_bad > 0 else "#067647",
            fontweight="bold",
        )
...
def generate_stock_report_with_four_dim(
    ...
    """
    todo 26-09-09
    复用旧绘图引擎生成四维整合版 PDF，并在首页/个股页明确标注数据是否最新可信。
    """
    ...
    with legacy_reporter.PdfPages(output_path) as pdf:
        ...
        for ticker in target_stocks:
            ...
            dq_expected = dq.get("expected_trade_date", "N/A")
            dq_actual = dq.get("actual_trade_date", "N/A")
            if dq_status not in {"OK", "RETRY_RECOVERED"}:
                fig.text(
                    0.5,
                    0.032,
                    f"⚠ 数据状态 {dq_status}：预期 {dq_expected}，实际可信 {dq_actual}；"
                    "本页技术指标不得视为预期交易日的最新信号。",
                    ha="center",
                    fontsize=9.5,
                    color="#b42318",
                    fontweight="bold",
                ...
                fig.text(
                    0.5,
                    0.032,
                    f"数据状态：重拉后恢复 | 最新可信交易日 {dq_actual}",
                    ha="center",
                    fontsize=9.2,
                    color="#b54708",
                )
...
def run_pipeline(is_need_download: bool = True, refresh_risk: bool = True) -> Optional[Path]:
    """
    todo 26-09-09
    四维整合主流水线：下载阶段返回数据质量状态；若跳过下载则读取最近一次状态，并传递给最终 PDF。
    """
    print("=" * 68)
