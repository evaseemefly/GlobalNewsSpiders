...
def print_top_results(results: list[dict], top_n: int = 10) -> None:
    print(f"🏆 Top {top_n} 最优参数组合 (按 Calmar、年化、平均仓位排序):")
    print("-" * 150)
    print(
...
def main() -> None:
    print("=== 🚀 VOO Hybrid 参数自动搜索（Grid Search, Revised）===")
    ...
    # Revised: Calmar 相同时，优先年化收益；仍相近时，优先更低平均仓位。
    results = sorted(
        results,
    ...
    best = results[0]
    dd_peak = best['dd_peak'].strftime('%Y-%m-%d') if pd.notna(best['dd_peak']) else 'N/A'
    dd_trough = best['dd_trough'].strftime('%Y-%m-%d') if pd.notna(best['dd_trough']) else 'N/A'

    print(
        f"\n🎯 最优参数（VOO Revised）：MA{best['ma_len']} | US10Y>{best['us10y_th']} | "
        f"VIX>{best['vix_th']} | RSI<{best['rsi_th']} | 风险仓位 {best['risk_pos']}"
    )
    print(
        f"年化: {best['cagr'] * 100:.2f}% | 最大回撤: {best['max_dd'] * 100:.2f}% "
        f"({dd_peak} -> {dd_trough}) | Calmar: {best['calmar']:.2f} | "
        f"平均仓位: {best['avg_exposure']:.1f}%"
    )

    print_regime_checks(df, best)
    print_oos_top10(df, results, top_n=10)

    output_pic_path = Path(__file__).parent / "09_best_hybrid_voo_top10_revised.png"
    plot_top10(df, results, output_pic_path)
    print(f"\n🖼️ Top 10 策略对比图已保存: {output_pic_path}")
