...
# -*- coding: utf-8 -*-
"""
VOO Hybrid V4.2 执行层参数回测 / Grid Search
...
1) 收敛 VIX 状态阈值
   ...
   - VOO Panic Drop 以 -2.0% 为中心，保留 -1.5% / -2.5% 做敏感性测试。
...
# todo:26-06-08: V4.2 基于 VIX regime 验证结果收敛状态参数，并加入交易摩擦成本。
"""
...
RESULT_CSV_PATH = OUTPUT_DIR / "voo_v4_2_gridsearch_results.csv"
STATE_SUMMARY_CSV_PATH = OUTPUT_DIR / "voo_v4_2_state_threshold_sensitivity_summary.csv"
BEST_JSON_PATH = OUTPUT_DIR / "voo_v4_2_best_params.json"
TOP10_PIC_PATH = OUTPUT_DIR / "voo_v4_2_top10_nav_compare.png"
BEST_PIC_PATH = OUTPUT_DIR / "voo_v4_2_best_vs_v3_baseline.png"
...
V3_FIXED_PARAMS = {
    ...
    # V3 已验证核心参数，不在 V4.2 中重新搜索，避免过拟合。
    "ma_len": 100,
    "us10y_th": 0.15,
    "vix_th": 45,
    "rsi_th": 30,
...
# - Panic Drop 以 -2.0% 为中心，小范围测试 -1.5% / -2.5%。
...
STATE_PARAM_GRID = {
    ...
    # VOO Panic Drop 核心阈值：-2.0%；保留 -1.5% / -2.5% 做鲁棒性测试。
...
V4_EXECUTION_GRID = {
    ...
    "risk_warning_gap_buy_pct": [0.20, 0.25, 0.33],

    # 风险修复条件：HYG > MA60 且 VIX < 18 / 20。
...
def run_v4_2_execution_strategy(df, base_params, state_params, execution_params):
    """
    V4.2 执行层回测。
    """
    p = dict(base_params)
...
def score_v4_2(metrics, baseline_metrics):
    """
    辅助评分：主排序仍看 Calmar。
...
def plot_top10(df, baseline_nav, results):
    ...
    ax1.set_title("VOO V4.2 执行层参数搜索 Top10 vs V3 Baseline", fontsize=16, fontweight="bold")
    ...
    best = results[0]
    ax2.plot(df.index, best["position"], label="Best V4.2 Position", color="#34495e", linewidth=1.8)
    ax2.set_ylim(0, 1.05)
...
def plot_best_vs_baseline(df, baseline, best_result):
    ...
    ax1.plot(df.index, buy_hold_nav, label="Buy & Hold VOO", color="gray", alpha=0.55, linewidth=2)
    ax1.plot(df.index, baseline_nav, label="V3 Fixed Baseline", color="black", linewidth=2.5)
    ax1.plot(df.index, best_nav, label="Best V4.2 Execution", color="#e74c3c", linewidth=2.5)
    ax1.set_yscale("log")
    ax1.set_title("Best V4.2 Execution vs V3 Baseline", fontsize=16, fontweight="bold")
    ax1.legend(loc="upper left")
    ax1.grid(True, alpha=0.4)

    ax2.plot(df.index, baseline_pos, label="V3 Position", color="black", linewidth=1.5)
    ax2.plot(df.index, best_pos, label="V4.2 Position", color="#e74c3c", linewidth=1.8)
    ax2.set_ylim(0, 1.05)
    ax2.set_ylabel("Position")
    ax2.legend(loc="upper left")
    ax2.grid(True, alpha=0.4)
...
def main():
    print("=== 🚀 VOO Hybrid V4.2 执行层参数搜索（Grid Search）===")
    ...
    print(f"\n开始搜索 V4.2 状态 + 执行层参数... 共 {total} 种组合\n")
    ...
    for state_combo in product(*state_values):
        ...
        for execution_combo in product(*execution_values):
            ...
            res = run_v4_2_execution_strategy(
                df=df,
            ...
            score = score_v4_2(metrics, baseline_metrics)
    ...
    print("🏆 Top 20 V4.2 参数组合（主排序：Calmar；Score 辅助）:")
    print("-" * 210)
    print(
        f"{'排名':<4} "
    ...
    print(
        f"\n📈 V4.2 Best: CAGR {best['cagr'] * 100:.2f}% | "
    ...
    print(f"\n💾 V4.2 全部搜索结果已保存: {RESULT_CSV_PATH.name}")
    ...
    print(f"💾 状态阈值敏感性摘要已保存: {STATE_SUMMARY_CSV_PATH.name}")
    ...
    best_json = {
        ...
        "v4_2_best_params": {k: best[k] for k in state_keys + execution_keys},
        "v4_2_best_metrics": {
        ...
        "notes": [
            "V4.2 keeps V3 core parameters fixed.",
            "V4.2 narrows VIX state thresholds according to regime validation.",
            "V4.2 uses transaction costs and annualized turnover.",
    ...
    print(f"💾 V4.2 最优参数 JSON 已保存: {BEST_JSON_PATH.name}")
