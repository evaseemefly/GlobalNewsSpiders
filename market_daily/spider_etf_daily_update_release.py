...
def download_latest_data(last_date: pd.Timestamp) -> pd.DataFrame:
    ...
    # todo 26-06-17: 先剔除 BTC-only 日期，再要求核心列完整；不再用 ffill 伪造 OHLCV。
    ...
    # todo 26-06-17: 对所有核心 OHLCV 列严格要求完整，避免单个资产缺失时被前值伪造。
    required_cols = [
        f'{name}_{field}'
        for name in TICKERS
    ...
    if missing_cols:
        print(f"❌ 本次更新缺少核心列: {missing_cols}，已放弃写入主表")
        return pd.DataFrame()

    before_dropna = len(merged)
    merged = merged.dropna(subset=required_cols)
    dropped_incomplete = before_dropna - len(merged)
    if dropped_incomplete:
        print(f"🧹 已剔除 {dropped_incomplete} 条存在缺失资产数据的记录")
